...
function TaggedTemplateExpression(path, state, t, options) {
  ...
  var className = options.displayName ? (0, _toValidCSSIdentifier.default)(displayName) + "_" + slug : slug; // Optionall the className can be defined by the user

  if (typeof options.classNameSlug === 'string') {
    var classNameSlug = options.classNameSlug; // Available variables for the square brackets used in `classNameSlug` options

    var classNameSlugVars = {
      hash: slug,
      title: displayName
    }; // Variables that were used in the config for `classNameSlug`

    var optionVariables = classNameSlug.match(/\[.*?\]/g) || [];
    var cnSlug = classNameSlug;

    for (var i = 0, l = optionVariables.length; i < l; i++) {
      var v = optionVariables[i].slice(1, -1); // Remove the brackets around the variable name
